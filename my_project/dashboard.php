...
/* Main content */
.main{flex:1;padding:30px;display:flex;
...
.card{background:var(--card-dark);
  padding:25px;
  background: linear-gradient(145deg,#1e293b , #4c5c74ff);
  border-radius:15px;
box-shadow:0 8px 20px rgba(0,0,0,5);text-align:center}
...
/* Tasks List */
.task-item{background:var(--card-dark);padding:20px;
...
function logout(){
  ...
  f.append('csrf_token',csrf_token); // تأكيد الـ CSRF
  fetch('backend.php',{method:'POST',body:f})
  .then(r=>r.json())
  .then(d=>{
    if(d.success){
