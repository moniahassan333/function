...
<style>
body{font-family:sans-serif;background: #2f375eff;margin:0;padding:0;display:flex;justify-content:center;align-items:center;height:100vh}
.container{width:350px;padding:30px;border-radius:15px;box-shadow:0 0 20px rgba(0,0,0,0.1);background:#fff}
h2{text-align:center;color:#1e3a8a}
input{width:100%;padding:10px;margin:10px 0;border-radius:8px;border:1px solid #404284ff}
button{width:100%;padding:10px;background:#1e3a8a;color:#fff;border:none;border-radius:8px;margin-top:10px;cursor:pointer}
.toggle{background:#2563eb;margin-top:5px}
.toast{position:fixed;bottom:20px;left:50%;transform:translateX(-50%);padding:10px 20px;background:#1e3a8a;color:#fff;border-radius:10px;display:none}
</style>
</head>
<body>
...
function toggleForm(){
  ...
  document.getElementById('registerDiv').style.display = document.getElementById('registerDiv').style.display==='none'?'block':'none';
}
...
function login(){
  ...
  fetch('backend.php',{
    method:'POST',
    body:f,
    credentials:'include' // مهم جدًا لتفعيل الجلسة بدون تحديث
  })
  .then(r=>r.json())
  .then(d=>{
    toast(d.message);
